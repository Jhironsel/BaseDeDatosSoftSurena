...
SET TERM ^ ;
CREATE PROCEDURE SP_VALIDAR_DIRECCION (
    I_ID_DIRECCION D_ID,
    I_ID_PERSONA D_ID,
    I_ID_PROVINCIA D_ID,
    I_ID_MUNICIPIO D_ID,
...
AS SELECT 
     d.ID, d.ID_CLIENTE, d.ID_FACTURA, d.CONCEPTO, d.MONTO, d.FECHA, d.HORA,
     DECODE(
          d.ESTADO, 
               'i', 'Inicial', 
               'c', 'Credito', 
...
*/;
CREATE VIEW GET_PERMISOS_ASIGNADOS (NOMBRE_RELACION, GRANT_OPTION, 
     ID_TIPO_OBJ, USUARIO, DESCRIPCION)
AS SELECT 
...
SELECT 
     r.RDB$PROCEDURE_ID, 
     TRIM(r.RDB$PROCEDURE_NAME),
     TRIM(r.RDB$OWNER_NAME), 
     r.RDB$SQL_SECURITY, 
     TRIM(r.RDB$DESCRIPTION)
...
/*
     Me permite obtener todos los roles del sistema registrados.
*/;
CREATE VIEW VS_USUARIOS (USERNAME, PNOMBRE, SNOMBRE, APELLIDOS, 
...
CREATE VIEW V_M_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, 
     ...
     USER_NAME)
AS SELECT LPAD(ID, 7, 0), ID_CLIENTE, ID_CONTACTOS_TEL, ID_CONTACTOS_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO,
...
BEGIN
     ...
     EXECUTE PROCEDURE SP_VALIDAR_DIRECCION (0, :I_ID_PERSONA,
     :I_ID_PROVINCIA, :I_ID_MUNICIPIO, :I_ID_DISTRITO_MUNICIPAL, 
     :I_ID_CODIGO_POSTAL);
     
     INSERT INTO V_CONTACTOS_DIRECCIONES(ID_PERSONA, ID_PROVINCIA, ID_MUNICIPIO,
          ID_DISTRITO_MUNICIPAL, ID_CODIGO_POSTAL, DIRECCION)
     VALUES (:I_ID_PERSONA, :I_ID_PROVINCIA, :I_ID_MUNICIPIO, :I_ID_DISTRITO_MUNICIPAL, 
...
AS
BEGIN
     EXECUTE PROCEDURE SP_VALIDAR_DIRECCION (:I_ID, 0,
     :I_ID_PROVINCIA, :I_ID_MUNICIPIO, :I_ID_DISTRITO_MUNICIPAL, 
     :I_ID_CODIGO_POSTAL);
...
SET TERM ^ ;
ALTER PROCEDURE SP_VALIDAR_DIRECCION (
    I_ID_DIRECCION D_ID,
    I_ID_PERSONA D_ID,
    I_ID_PROVINCIA D_ID,
    I_ID_MUNICIPIO D_ID,
...
AS
BEGIN
     --Verificando el codigo de la direccion del cliente. UPDATE
     IF((SELECT (1) 
          FROM V_CONTACTOS_DIRECCIONES 
          WHERE ID = :I_ID_DIRECCION) IS NULL AND :I_ID_DIRECCION >= 0)THEN
          EXCEPTION E_DIRECCION_NO_ENCONTRADA;
     
     --Verificando el codigo del cliente. INSERT
     IF((SELECT (1) 
          FROM GET_CLIENTES_SB c 
          WHERE c.ID = :I_ID_PERSONA) IS NULL AND :I_ID_PERSONA >= 0)THEN
          EXCEPTION E_CLIENTE_NO_ENCONTRADO;     
     
     --Verificando el codigo de la provincia.
     IF((SELECT (1) 
          FROM V_PROVINCIAS 
          WHERE ID = :I_ID_PROVINCIA) IS NULL AND :I_ID_PROVINCIA >= 0)THEN
               EXCEPTION E_PROVINCIA_NO_DEFINIDA;
     
     --Verificando el codigo del MUNICIPIOS sea mayor o igual a cero
     IF((SELECT (1) 
          FROM V_MUNICIPIOS 
          WHERE ID = :I_ID_MUNICIPIO) IS NULL AND :I_ID_MUNICIPIO >= 0)THEN BEGIN
     ...
     END ELSE BEGIN
          ...
          WHERE ID = :I_ID_MUNICIPIO AND 
                IDPROVINCIA = :I_ID_PROVINCIA) IS NULL)THEN
     ...
     IF((SELECT (1) 
          ...
          WHERE ID = :I_ID_DISTRITO_MUNICIPAL ) IS NULL)THEN BEGIN
          
               EXCEPTION E_DISTRITO_M_NO_DEFINIDO;
          
     END ELSE BEGIN
          --Verificamos que el DISTRITOS_MUNICIPALES y MUNICIPIOS coincidan. 
          IF(:I_ID_DISTRITO_MUNICIPAL > 0 AND (SELECT (1) 
          FROM V_DISTRITOS_MUNICIPALES 
          WHERE ID = :I_ID_DISTRITO_MUNICIPAL AND IDMUNICIPIO = :I_ID_MUNICIPIO) IS NULL)THEN
...
GRANT EXECUTE
 ON PROCEDURE SP_VALIDAR_DIRECCION TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_VALIDAR_DIRECCION TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
