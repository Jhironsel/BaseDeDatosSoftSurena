...
from V_PRODUCTOS P 
join V_CATEGORIAS R on R.ID = P.ID_CATEGORIA and P.ESTADO
...
CREATE VIEW GET_DIRECCION_BY_ID (ID, ID_PERSONA, ID_PROVINCIA, PROVINCIA, ID_MUNICIPIO, MUNICIPIO, 
     ID_DISTRITO_MUNICIPAL, DISTRITO_MUNICIPAL, ID_CODIGO_POSTAL, CODIGO_POSTAL, DIRECCION, FECHA)
...
CREATE VIEW GET_D_FACTURAS (ID_FACTURA, ID_LINEA, ID_PRODUCTO, DESCRIPCION, PRECIO, CANTIDAD, TOTAL)
...
CREATE VIEW GET_ESTUDIANTES_SV (ID, MATRICULA, ID_PADRE, CEDULA, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, FECHA_NACIMIENTO, 
     ESTADO)
...
CREATE VIEW GET_PACIENTES (ID, ID_PADRE, ID_ARS, NONSS, ID_TIPO_SANGRE, CEDULA, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, 
...
CREATE VIEW GET_TEMPORALES (ID_FACTURA, NOMBRE_TEMP, PNOMBRE, SNOMBRE, APELLIDOS, ID_TURNO, EFECTIVO, CAMBIO, 
     FECHA_HORA, ESTADO_FACTURA, MONTO, ID_CLIENTE, USER_NAME)
...
AS SELECT r.ID, r.ID_PACIENTE, r.FECHA_HORA_CREADA, r.FECHA_HORA_VISTA,
     ...
     r.T_TRIGLICERIDOS, r.T_TSH, r.T_TUBERCULINA, r.T_UREA, r.OTROS, r.USER_NAME,
     r.ROL
FROM ANALISIS r;
CREATE VIEW V_ANTECEDENTES (ID, ID_CONSULTA, DESCRIPCION)
AS SELECT r.ID, r.ID_CONSULTA, r.DESCRIPCION
FROM ANTECEDENTES r;
CREATE VIEW V_ARS (ID, DESCRIPCION, COVERCONSULTAPORC, ESTADO, USER_NAME, ROL)
AS SELECT ID, DESCRIPCION, COVERCONSULTAPORC, ESTADO, USER_NAME, ROL
FROM ARS;
...
CREATE VIEW V_DATOS_NACIMIENTO (ID, PESONACIMIENTOKG, ALTURA, PERIMETRO_CEFALICO, CESAREA, TIEMPOGESTACION, MC, 
     USER_NAME)
AS SELECT ID, PESONACIMIENTOKG, ALTURA, PERIMETRO_CEFALICO, CESAREA, TIEMPOGESTACION, MC, USER_NAME
FROM DATOS_NACIMIENTO;
...
AS SELECT r.ID, r.ID_CLIENTE, r.ID_FACTURA, r.CONCEPTO, r.MONTO,r.FECHA, r.HORA,
          r.ESTADO
FROM DEUDAS r;
CREATE VIEW V_DIRECCIONES (ID, ID_PERSONA, ID_PROVINCIA, ID_MUNICIPIO, ID_DISTRITO_MUNICIPAL, ID_CODIGO_POSTAL, 
...
FROM E_S_SYS;
CREATE VIEW V_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, ID_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO, FECHA_HORA, 
     TOTAL, EFECTIVO, CAMBIO, ESTADO_FACTURA, NOMBRE_TEMP, USER_NAME)
...
AS SELECT r.ID, r.PERSONA, r.PNOMBRE, r.SNOMBRE,
     ...
     r.FECHA_HORA_ULTIMO_UPDATE, r.ESTADO
FROM PERSONAS r;
...
AS SELECT r.ID, r.IDCONSULTA, r.FECHA
FROM RECETAS r;
...
CREATE VIEW V_TANDAS (ID, ANNO_INICIAL, ANNO_FINAL, HORA_INICIO, HORA_FINAL, LUNES, MARTES, MIERCOLES, JUEVES, VIERNES, 
     SABADOS, DOMINGOS, CANTIDAD_ESTUDIANTES, EDAD_MINIMA, EDAD_MAXIMA, CON_EDAD, ESTADO)
...
CREATE VIEW V_TURNOS (ID, TURNO_USUARIO, FECHA_HORA_INICIO, FECHA_HORA_FINAL, ESTADO, ROL, USER_NAME)
AS SELECT ID, TURNO_USUARIO, FECHA_HORA_INICIO, FECHA_HORA_FINAL, ESTADO, ROL, USER_NAME
FROM TURNOS;
...
BEGIN
     ...
     */
     ...
     IF(:I_ID_DIRECCIONES <> 0 AND (SELECT (1) 
          FROM V_DIRECCIONES e
          WHERE e.ID = :I_ID_DIRECCIONES AND 
                e.ID_PERSONA = :I_ID_CLIENTE AND 
                e.ESTADO) IS NULL) THEN
          EXCEPTION E_DIRECCION_INACTIVO;
          
     INSERT INTO V_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, ID_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO, TOTAL, 
               EFECTIVO, ESTADO_FACTURA, NOMBRE_TEMP)
     VALUES (-1, :I_ID_CLIENTE, :I_ID_CONTACTOS_TEL, :I_ID_DIRECCIONES, :I_ID_CONTACTOS_EMAIL, :I_ID_TURNO, :I_TOTAL, 
...
ALTER TABLE CONSULTAS ADD CONSTRAINT FK_CONSULTAS_0
  FOREIGN KEY (ID_PACIENTE) REFERENCES PACIENTES (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GET_CLIENTES_GENERALES TO ROLE RDB$ADMIN WITH GRANT OPTION;
