...
(
  ID_PADRE_O_MADRE D_ID NOT NULL,
  ID_HIJO D_ID NOT NULL,
  CONSTRAINT PK_RELACION_FAMILIAR_0 PRIMARY KEY (ID_PADRE_O_MADRE,ID_HIJO)
);
...
WHERE r.ID >= 0
WITH CHECK OPTION;
CREATE VIEW V_ARS (ID, DESCRIPCION, COVERCONSULTAPORC, ESTADO, CANTIDAD_REGISTRO, USER_NAME, ROL)
AS SELECT ID, DESCRIPCION, COVERCONSULTAPORC, ESTADO, CANTIDAD_REGISTRO, USER_NAME, ROL
FROM ARS
WHERE ID >= 0
WITH CHECK OPTION;
...
CREATE VIEW V_CONTACTOS_DIRECCIONES (ID, ID_PERSONA, ID_PROVINCIA, ID_MUNICIPIO, ID_DISTRITO_MUNICIPAL, 
     ID_CODIGO_POSTAL, DIRECCION, FECHA, ESTADO)
AS SELECT ID, ID_PERSONA, ID_PROVINCIA, ID_MUNICIPIO,
...
AS SELECT ID, ID_PERSONA, EMAIL, FECHA, ESTADO
FROM CONTACTOS_EMAIL
WHERE ID >= 0 
WITH CHECK OPTION;
CREATE VIEW V_CONTACTOS_TEL (ID, ID_PERSONA, TELEFONO, TIPO, FECHA, ESTADO)
AS SELECT ID, ID_PERSONA, TELEFONO, TIPO, FECHA, ESTADO
FROM CONTACTOS_TEL 
WHERE ID >= 0 
WITH CHECK OPTION;
...
BEGIN
     ...
     */
     ...
     IF(( SELECT CANTIDAD 
          FROM V_RECCOUNT 
          WHERE TABLA LIKE 'V_PERSONAS'||' '||:I_ESTADO) > 0)THEN
               EXECUTE PROCEDURE SP_RECCOUNT_ESTADO('D', 'V_PERSONAS', :I_ESTADO);
          
     IF(( SELECT CANTIDAD 
          FROM V_RECCOUNT 
          WHERE TABLA LIKE 'GET_CLIENTES_ESTADO_SB'||' '||:I_ESTADO) > 0)THEN
               EXECUTE PROCEDURE SP_RECCOUNT_ESTADO('D', 'GET_CLIENTES_ESTADO_SB', :I_ESTADO);
     
     IF(( SELECT CANTIDAD 
          FROM V_RECCOUNT 
          WHERE TABLA LIKE 'GET_CLIENTES'||' '||:I_ESTADO) > 0)THEN
               EXECUTE PROCEDURE SP_RECCOUNT_ESTADO('D', 'GET_CLIENTES', :I_ESTADO);
               
     IF(( SELECT CANTIDAD 
          FROM V_RECCOUNT 
          WHERE TABLA LIKE 'GET_CLIENTES_SB'||' '||:I_ESTADO) > 0)THEN
               EXECUTE PROCEDURE SP_RECCOUNT_ESTADO('D', 'GET_CLIENTES_SB', :I_ESTADO);
     
     /*Guardamos cantidad total de los registros en padres*/
     IF(( SELECT CANTIDAD 
          FROM V_RECCOUNT 
          WHERE TABLA LIKE 'V_PERSONAS') > 0)THEN
               EXECUTE PROCEDURE SP_RECCOUNT('D', 'V_PERSONAS');
               
     IF(( SELECT CANTIDAD 
          FROM V_RECCOUNT 
          WHERE TABLA LIKE 'V_CLIENTES') > 0)THEN
               EXECUTE PROCEDURE SP_RECCOUNT('D', 'V_PERSONAS_CLIENTES');
               
     IF(( SELECT CANTIDAD 
          FROM V_RECCOUNT 
          WHERE TABLA LIKE 'V_GENERALES') > 0)THEN
               EXECUTE PROCEDURE SP_RECCOUNT('D', 'V_GENERALES');
               
     IF(( SELECT CANTIDAD 
          FROM V_RECCOUNT 
          WHERE TABLA LIKE 'V_DIRECCIONES') > 0)THEN
...
ALTER TABLE PERSONAS_ESTUDIANTES_ATR ADD CONSTRAINT FK_PERSONAS_ESTUDIANTES_ATR_0
  FOREIGN KEY (ID) REFERENCES PERSONAS (ID) ON DELETE NO ACTION;
...
ALTER TABLE RECETAS ADD CONSTRAINT FK_RECETAS_1
  FOREIGN KEY (IDCONSULTA) REFERENCES CONSULTAS (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
ALTER TABLE RELACION_FAMILIAR ADD CONSTRAINT FK_RELACION_FAMILIAR_0
  FOREIGN KEY (ID_PADRE_O_MADRE) REFERENCES PERSONAS (ID) ON UPDATE CASCADE ON DELETE NO ACTION;
ALTER TABLE RELACION_FAMILIAR ADD CONSTRAINT FK_RELACION_FAMILIAR_1
  FOREIGN KEY (ID_HIJO) REFERENCES PERSONAS (ID) ON UPDATE CASCADE ON DELETE NO ACTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON RELACION_FAMILIAR TO  SYSDBA WITH GRANT OPTION;
...
GRANT SELECT
 ON V_CONTACTOS_TEL TO ROLE RRR_SECRETARIA;
