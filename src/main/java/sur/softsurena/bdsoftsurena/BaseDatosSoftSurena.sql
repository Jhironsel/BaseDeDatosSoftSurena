...
CREATE VIEW GET_CLIENTES (ID, ID_PROVINCIA, ID_MUNICIPIO, ID_DISTRITO_MUNICIPAL, 
     ID_CODIGO_POSTAL, ID_TIPO_SANGRE, PERSONA, CEDULA, PNOMBRE, SNOMBRE, APELLIDOS, 
     SEXO, DIRECCION, FECHA_NACIMIENTO, FECHA_INGRESO, FECHA_HORA_ULTIMO_UPDATE, ESTADO, 
     ESTADO_CIVIL, ID_USUARIO, ROL_USUARIO)
AS /*
...
INNER JOIN V_PERSONAS r ON r.ID = c.ID
INNER JOIN V_GENERALES g ON g.ID_PERSONA = c.ID;
CREATE VIEW GET_CLIENTES_GENERALES (ID, CEDULA)
AS SELECT r.ID, g.CEDULA
FROM V_CLIENTES r
INNER JOIN V_GENERALES g ON g.ID_PERSONA = r.ID;
CREATE VIEW GET_CLIENTES_SB (ID, CEDULA, PERSONA, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, 
...
INNER JOIN V_PERSONAS r ON r.ID = c.ID
INNER JOIN V_GENERALES g ON g.ID_PERSONA = c.ID;
CREATE VIEW GET_DEUDAS (ID, ID_CLIENTE, ID_FACTURA, CONCEPTO, MONTO, FECHA, HORA, ESTADO, 
...
CREATE VIEW GET_DIRECCION_BY_ID (ID, ID_PERSONA, ID_PROVINCIA, PROVINCIA, ID_MUNICIPIO, 
     MUNICIPIO, ID_DISTRITO_MUNICIPAL, DISTRITO_MUNICIPAL, ID_CODIGO_POSTAL, 
     CODIGO_POSTAL, DIRECCION, FECHA)
...
CREATE VIEW GET_D_FACTURAS (ID_FACTURA, ID_LINEA, ID_PRODUCTO, DESCRIPCION, PRECIO, 
     CANTIDAD, TOTAL)
...
CREATE VIEW GET_ESTUDIANTES_SV (ID, MATRICULA, ID_PADRE, CEDULA, PNOMBRE, SNOMBRE, 
...
CREATE VIEW GET_PACIENTES (ID, ID_PADRE, ID_ARS, NONSS, ID_TIPO_SANGRE, CEDULA, PNOMBRE, 
     SNOMBRE, APELLIDOS, SEXO, FECHA_NACIMIENTO, FECHA_INGRESO, FECHA_HORA_ULTIMO_UPDATE, 
     ESTADO, ID_USUARIO)
...
CREATE VIEW GET_PRODUCTOS (ID, IDCATEGORIA, DESC_CATEGORIA, IMAGEN_CATEGORIA, CODIGO, 
     DESCRIPCION, IMAGEN_TEXTO, NOTA, FECHA_CREACION, ESTADO)
...
AS select RDB$ROLE_NAME --iif(RDB$ROLE_NAME = 'RDB$ADMIN', 'RRR_ADMINISTRADOR', RDB$ROLE_NAME)
from RDB$ROLES r
where trim(r.rdb$role_name) not like 'RRR_SOFTSURENA';
CREATE VIEW GET_TEMPORALES (ID_FACTURA, NOMBRE_TEMP, PNOMBRE, SNOMBRE, APELLIDOS, 
     IDUSUARIO, ID_TURNO, EFECTIVO, CAMBIO, FECHA_HORA, ESTADO_FACTURA, MONTO, ID_CLIENTE)
AS SELECT f.ID, f.NOMBRE_TEMP, p.PNOMBRE, p.SNOMBRE, p.APELLIDOS, 
     f.IDUSUARIO, f.ID_TURNO, f.EFECTIVO, f.CAMBIO, f.FECHA_HORA, f.ESTADO_FACTURA,
...
GROUP by f.ID, f.NOMBRE_TEMP, p.PNOMBRE, p.SNOMBRE, p.APELLIDOS, 
          f.IDUSUARIO, f.ID_TURNO, f.EFECTIVO, f.CAMBIO, f.FECHA_HORA, f.ESTADO_FACTURA, f.ID_CLIENTE;
CREATE VIEW V_ANALISIS (ID, ID_PACIENTE, FECHA_HORA_CREADA, FECHA_HORA_VISTA, T_BHCG, 
     T_EMB_ORINA, T_EMB_SANGRE, T_ANT_AUSTRALIANO_BBSAG, T_CLAMIDIA_IGA, T_FTA_ABS, 
     T_HIV, T_VDRL, T_ACIDO_URICO, T_ANT_FEBRILES, T_ASO_LATEX, T_BILIRRUBINA, 
     T_COLESTEROL_TOTAL, T_COLESTEROL_HDL, T_COLESTEROL_HDL_LDL, T_COPROLOGICO, 
     T_CREATININA_SUERO, T_CULTIVO_FARINGE, T_CULTIVO_HECES, T_CULTIVO_HERIDA_AEROBICO, 
     T_CULTIVO_OIDO, T_CULTIVO_SANGRE, T_CULTIVO_SEMEN, T_CULTIVO_URETRA, 
     T_CULTIVO_VAGINA, T_ELECTRO_HEMOGLOBINA, T_ERITROSEDIMENTACION, T_ESPERMATOGRAMA, 
     T_ESTRADIOL, T_ESTROGENO_TOTALES, T_FACTOR_REUMATOIDE, T_FALCEMIA, 
     T_FOSFATASA_ALCALINA, T_FSH, T_GLICEMIA, T_HEMOGLO_GLUCOSILADA, T_HEMOGRAMA, 
     T_HEPATITIS_A, T_HEPATITIS_C, T_KOH, T_LH, T_ORINA, T_PCR, T_PROGESTERONA, 
     T_PROLACTINA, T_PROTEINAS_TOTALES, T_PSA_TOTAL, T_PSA_LIBRE, T_SANG_OCULT_HECES, 
     T_SGOT_TGO, T_SGOT_TGP, T_T3, T_T4, T_T4_LIBRE, T_COOMBS_DIRECTO, 
     T_COOMBS_INDIRECTO, T_TESTOSTERONA, T_TIPIFICACION_SANGUINEA, T_TOXOPLAS_IGG, 
     T_TOXOPLAS_IGM, T_TRIGLICERIDOS, T_TSH, T_TUBERCULINA, T_UREA, OTROS, IDUSUARIO, ROL)
AS SELECT r.ID, r.ID_PACIENTE, r.FECHA_HORA_CREADA, r.FECHA_HORA_VISTA,
     ...
     r.ROL
FROM ANALISIS r;
CREATE VIEW V_ANTECEDENTES (ID, ID_PACIENTE, ID_DOCTOR, FECHA, DESCRIPCION, USUARIO, ROL)
...
FROM ASEGURADOS r;
CREATE VIEW V_CATEGORIAS (ID, DESCRIPCION, IMAGEN_TEXTO, FECHA_CREACION, ESTADO, IDUSUARIO
     )
...
CREATE VIEW V_CONSULTAS (ID, ID_PACIENTE, ID_CONTROL_CONSULTA, FECHA, TURNO, ESTADO, 
     IDUSUARIO)
AS SELECT r.ID, r.ID_PACIENTE, r.ID_CONTROL_CONSULTA, 
          r.FECHA, r.TURNO, r.ESTADO,r.IDUSUARIO
FROM CONSULTAS r;
CREATE VIEW V_CONSULTAS_APROBADAS (ID, COD_AUTORIZACION, COSTO, DESCUENTO, IDUSUARIO, 
     TOTALCOSTO)
AS SELECT r.ID, r.COD_AUTORIZACION, r.COSTO, r.DESCUENTO, r.IDUSUARIO, r.TOTALCOSTO
FROM CONSULTAS_APROBADAS r;
CREATE VIEW V_CONTACTS_EMAIL (ID, ID_PERSONA, EMAIL, FECHA, ESTADO)
AS SELECT r.ID, r.ID_PERSONA, r.EMAIL, r.FECHA, r.ESTADO
FROM CONTACTOS_EMAIL r;
CREATE VIEW V_CONTACTS_TEL (ID, ID_PERSONA, TELEFONO, TIPO, FECHA, ESTADO)
AS SELECT r.ID, r.ID_PERSONA, r.TELEFONO, r.TIPO, r.FECHA, r.ESTADO
FROM CONTACTOS_TEL r;
CREATE VIEW V_CONTROL_CONSULTA (ID, USER_NAME, CANTIDAD_PACIENTE, DIA, INICIAL, FINAL, 
     IDUSUARIO, ESTADO)
AS SELECT r.ID, r.USER_NAME, r.CANTIDADPACIENTE, r.DIA, r.INICIAL, r.FINAL, r.IDUSUARIO, r.ESTADO
FROM CONTROL_CONSULTA r;
CREATE VIEW V_DATOS_NACIMIENTO (ID, PESONACIMIENTOKG, ALTURA, PERIMETRO_CEFALICO, 
     CESAREA, TIEMPOGESTACION, MC, IDUSUARIO)
...
CREATE VIEW V_DIRECCIONES (ID, ID_PERSONA, ID_PROVINCIA, ID_MUNICIPIO, 
     ID_DISTRITO_MUNICIPAL, ID_CODIGO_POSTAL, DIRECCION, FECHA, ESTADO)
AS SELECT r.ID, r.ID_PERSONA, r.ID_PROVINCIA, r.ID_MUNICIPIO,
     r.ID_DISTRITO_MUNICIPAL, r.ID_CODIGO_POSTAL, r.DIRECCION, r.FECHA, r.ESTADO
FROM DIRECCIONES r;
...
FROM D_RECETAS r;
CREATE VIEW V_ENTRADA_PRODUCTOS (ID, ID_PROVEDOR, COD_FACTURA, LINEA, ID_PRODUCTO, 
     ENTRADA, FECHAVECIMIENTO, ESTADO, USUARIO, ROL)
...
FROM ESTUDIANTE r;
CREATE VIEW V_E_S_SYS (ID_E_S_SYS, NOMBRE, TELEFONOS, FCHI, FCHA, FCHV, IDMAC, DIRECCION, 
     MENSAJE_FOOTER, LOGO)
AS SELECT ID_E_S_SYS, NOMBRE, TELEFONOS, FCHI, FCHA, FCHV, IDMAC,
     DIRECCION, MENSAJE_FOOTER, LOGO
FROM E_S_SYS;
CREATE VIEW V_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, ID_DIRECCIONES, 
     ID_CONTACTOS_EMAIL, ID_TURNO, FECHA_HORA, TOTAL, EFECTIVO, CAMBIO, ESTADO_FACTURA, 
     IDUSUARIO, NOMBRE_TEMP)
AS SELECT ID, ID_CLIENTE, ID_CONTACTOS_TEL, ID_DIRECCIONES, ID_CONTACTOS_EMAIL,
     ID_TURNO, FECHA_HORA, TOTAL, EFECTIVO, (r.EFECTIVO - r.TOTAL) AS CAMBIO, 
     ESTADO_FACTURA, IDUSUARIO, NOMBRE_TEMP
FROM FACTURAS r;
...
CREATE VIEW V_INSCRIPCION (ID, ID_ESTUDIANTE, ID_TANDA, PAGO, FECHA_INSCRIPCION, 
     ID_USUARIO, ROL)
AS SELECT ID, ID_ESTUDIANTE, ID_TANDA, PAGO, FECHA_INSCRIPCION, ID_USUARIO, ROL
FROM INSCRIPCIONES;
...
FROM PADRES r;
CREATE VIEW V_PERSONAS (ID, PERSONA, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, FECHA_NACIMIENTO, 
     FECHA_INGRESO, FECHA_HORA_ULTIMO_UPDATE, ESTADO, ID_USUARIO, ROL_USUARIO)
...
FROM PERSONAS r;
CREATE VIEW V_PRODUCTOS (ID, IDCATEGORIA, CODIGO, DESCRIPCION, IMAGEN_TEXTO, NOTA, 
     FECHA_CREACION, ESTADO, IDUSUARIO, ROL)
...
FROM SINTOMAS r;
CREATE VIEW V_TANDAS (ID, ANNO_INICIAL, ANNO_FINAL, HORA_INICIO, HORA_FINAL, LUNES, 
     MARTES, MIERCOLES, JUEVES, VIERNES, SABADOS, DOMINGOS, CANTIDAD_ESTUDIANTES, 
...
AS SELECT r.ID, r.DESCRIPCION
FROM TIPOS_SANGRE r;
CREATE VIEW V_TRANSACCION_ACTUAL (TNIDENTIFICADORTRANSACCION, TNIDENTIFICADORCONEXION, 
     TCESTADOTRANSACCION, TDFECHAHORA, TNULTIMATRANSACCION, 
     TNOLDESTINTERESTINGTRANSACTION, TNOLDESTACTIVETRANSACTION, TCMODOAISLAMIENTO, 
...
CREATE VIEW V_TURNOS (ID, TURNO_USUARIO, FECHA_HORA_INICIO, FECHA_HORA_FINAL, ESTADO, 
     IDUSUARIO, ROL)
AS SELECT ID, TURNO_USUARIO, FECHA_HORA_INICIO, FECHA_HORA_FINAL, ESTADO, IDUSUARIO, ROL
FROM TURNOS;
/******************* EXCEPTIONS *******************/
...
BEGIN
     /*IF(( SELECT (1) 
          ...
          EXCEPTION ERROR 'Usuario no tiene turno Activo.';

     INSERT INTO V_FACTURAS (ID_CLIENTE, ID_TURNO, EFECTIVO, CAMBIO, ESTADO_FACTURA, NOMBRE_TEMP)
     VALUES (:I_ID_CLIENTE, :I_ID_TURNO, :I_EFECTIVO, :I_CAMBIO, :I_ESTADO_FACTURA, :I_NOMBRE_TEMP);
     */
...
BEGIN
     ...
     IF(:I_ID_DIRECCIONES <> 0 AND 
          (SELECT (1) 
               FROM V_DIRECCIONES e 
               WHERE e.ID = :I_ID_DIRECCIONES AND e.ESTADO) IS NULL)THEN
                    EXCEPTION E_DIRECCION_INACTIVO;
     
     
     
     UPDATE OR INSERT INTO V_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, ID_DIRECCIONES,
     ID_CONTACTOS_EMAIL, ID_TURNO, TOTAL, EFECTIVO, ESTADO_FACTURA, NOMBRE_TEMP)
     VALUES(:I_ID, :I_ID_CLIENTE, :I_ID_CONTACTOS_TEL, :I_ID_DIRECCIONES, 
...
COMMENT ON COLUMN FACTURAS.ID_DIRECCIONES IS 'Es la direccion del cliente por defecto y es la asignada para la factura.';
...
ALTER TABLE FACTURAS ADD CONSTRAINT FK_FACTURAS_0
  FOREIGN KEY (ID_CONTACTOS_TEL) REFERENCES CONTACTOS_TEL (ID) ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE FACTURAS ADD CONSTRAINT FK_FACTURAS_1
  FOREIGN KEY (ID_CONTACTOS_EMAIL) REFERENCES CONTACTOS_EMAIL (ID) ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE FACTURAS ADD CONSTRAINT FK_FACTURAS_2
  FOREIGN KEY (ID_DIRECCIONES) REFERENCES DIRECCIONES (ID) ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE FACTURAS ADD CONSTRAINT FK_FACTURAS_3
  FOREIGN KEY (ID_TURNO) REFERENCES TURNOS (ID) ON UPDATE CASCADE ON DELETE CASCADE;
...
GRANT EXECUTE
 ON PROCEDURE SP_INSERT_GENERALES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON CLIENTES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON DATOS_NACIMIENTO TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON DIRECCIONES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON ESTUDIANTE TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON PACIENTES TO ROLE RDB$ADMIN WITH GRANT OPTION;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON PACIENTES TO  SYSDBA WITH GRANT OPTION;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON PADRES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON PROVEEDORES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GET_CLIENTES_ESTADO_SB TO ROLE RDB$ADMIN WITH GRANT OPTION;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GET_CLIENTES_ESTADO_SB TO  SYSDBA WITH GRANT OPTION;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GET_CLIENTES_GENERALES TO  SYSDBA WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_CLIENTES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_CONTACTS_EMAIL TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_CONTACTS_TEL TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_DATOS_NACIMIENTO TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_DIRECCIONES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_ESTUDIANTES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_PACIENTES TO ROLE RDB$ADMIN WITH GRANT OPTION;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_PACIENTES TO  SYSDBA WITH GRANT OPTION;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_PADRES TO ROLE RDB$ADMIN WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_PROVEEDORES TO ROLE RDB$ADMIN WITH GRANT OPTION;
