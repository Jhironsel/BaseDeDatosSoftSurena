...
AS SELECT TRIM(RDB$USER), TRIM(RDB$GRANTOR), TRIM(RDB$PRIVILEGE), RDB$GRANT_OPTION, 
       TRIM(RDB$RELATION_NAME), TRIM(RDB$FIELD_NAME), RDB$USER_TYPE, RDB$OBJECT_TYPE
...
AS
BEGIN
     EXECUTE STATEMENT 'ALTER ROLE RRR_CAJERO RENAME TO CAJERO';
END
^
SET TERM ; ^
...
BEGIN
     ...
     */
     ...
     IF((SELECT (1) FROM V_CATEGORIAS c WHERE c.ESTADO IS FALSE) = 1)THEN
