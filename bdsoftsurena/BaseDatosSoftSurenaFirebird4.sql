...
SET TERM ^ ;
CREATE PROCEDURE PERM_FRM_FACTURAR
SQL SECURITY DEFINER
AS 
BEGIN SUSPEND; 
END^
...
(
  ...
  ESTADO D_BOOLEAN_T,
  CONSTRAINT INTEG_2662 PRIMARY KEY (ID),
  CONSTRAINT INTEG_2663 UNIQUE (NOMBRE)
);
...
(
  ...
  ROL D_ROL,
  USER_NAME D_USER_NAME,
  CONSTRAINT INTEG_2616 PRIMARY KEY (ID)
);
...
(
  ...
  USER_NAME D_USER_NAME NOT NULL,
  ROL D_ROL,
  CONSTRAINT INTEG_2610 PRIMARY KEY (ID),
...
(
  ...
  USER_NAME D_USER_NAME NOT NULL,
  CONSTRAINT INTEG_2655 PRIMARY KEY (ID)
);
...
(
  ...
  NOTA D_VARCHAR_1024,
  CONSTRAINT INTEG_2640 PRIMARY KEY (ID),
  CONSTRAINT INTEG_2641 UNIQUE (NOMBRE_CORTO_ANALISIS),
  CONSTRAINT INTEG_2642 UNIQUE (NOMBRE_ANALISIS)
);
...
WHERE c.ID >= 0
/*
     Una categoria activa deberia de ser aquella donde los productos activos contienen una categoria.
     El CTE nos da aquella lista de id_categoria que tiene productos activos.
*/;
...
/*
     ...
     f.CAMBIO Este campo fue eliminado en esta oportunidad.
...
FROM D_DEUDAS_PAGAS 
WHERE ID >= 0;
CREATE VIEW V_D_ENTRADA_PRODUCTO (ID_M_ENTRADA_PRODUCTO, LINEA, ID_PRODUCTO, ENTRADA, COSTO, PRECIO, FECHA_VECIMIENTO, 
     OBSERVACION)
AS SELECT ID_M_ENTRADA_PRODUCTO, LINEA, ID_PRODUCTO, ENTRADA, COSTO, PRECIO, FECHA_VECIMIENTO, OBSERVACION
...
FROM MUNICIPIOS 
WHERE ID >= 0;
CREATE VIEW V_M_ENTRADA_PRODUCTOS (ID, ID_PROVEDOR, COD_FACTURA, FECHA_ENTRADA, ROL, USER_NAME)
AS SELECT ID, IDPROVEDOR, COD_FACTURA, FECHA_ENTRADA, ROL, USER_NAME
FROM M_ENTRADA_PRODUCTOS
WHERE ID >= 0;
CREATE VIEW V_M_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, ID_CONTACTOS_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO, 
     FECHA_HORA, TOTAL, EFECTIVO, ESTADO_FACTURA, NOMBRE_TEMP, USER_NAME)
AS SELECT ID, ID_CLIENTE, ID_CONTACTOS_TEL, ID_CONTACTOS_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO,
     FECHA_HORA, TOTAL, EFECTIVO, ESTADO_FACTURA, NOMBRE_TEMP, USER_NAME
FROM M_FACTURAS 
WHERE ID >= 0;
...
CREATE VIEW V_PRODUCTOS (ID, ID_CATEGORIA, CODIGO, DESCRIPCION, EXISTENCIA, IMAGEN_TEXTO, NOTA, FECHA_CREACION, ESTADO, 
     USER_NAME, ROL)
AS SELECT ID, ID_CATEGORIA, CODIGO, DESCRIPCION, EXISTENCIA, IMAGEN_TEXTO, NOTA, FECHA_CREACION, ESTADO, USER_NAME, ROL
FROM PRODUCTOS p
WHERE ID >= 0;
...
SET TERM ^ ;
ALTER PROCEDURE PERM_FRM_FACTURAR
SQL SECURITY DEFINER
...
BEGIN
     ...
     */
     ...
     INSERT INTO V_M_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, ID_CONTACTOS_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO, TOTAL, 
...
BEGIN
     ...
     FROM GET_CLIENTES_SB a 
     WHERE a.ID > 0 AND  
          TRIM(a.CEDULA) STARTING WITH TRIM(:criterioBusqueda) OR 
          TRIM(a.PNOMBRE) STARTING WITH TRIM(:criterioBusqueda) OR
          TRIM(a.SNOMBRE) STARTING WITH TRIM(:criterioBusqueda) OR
          TRIM(a.APELLIDOS) STARTING WITH TRIM(:criterioBusqueda) 
...
BEGIN
     ...
     WHERE a.ID >= 0 AND
           a.ID = :I_ID OR 
           a.CODIGO STARTING WITH :I_CODIGO OR 
           a.CODIGO CONTAINING :I_CODIGO OR 
           a.DESCRIPCION STARTING WITH :I_DESCRIPCION OR 
           a.DESCRIPCION CONTAINING :I_DESCRIPCION
     INTO 
          ID, 
          CODIGO, 
          DESCRIPCION 
...
';
COMMENT ON COLUMN PRODUCTOS.ESTADO IS 'Indica cual es el estado del producto en el sistema, si está activo o inactivo.
';
COMMENT ON COLUMN PRODUCTOS.FECHA_CREACION IS 'Se registra la fecha cuando fue creado en el sistema el producto.
';
COMMENT ON COLUMN PRODUCTOS.NOTA IS 'Una nota que puede indicar otras observaciones en el sistema del producto. 

En esta puede indicarse si el producto debe mantenerse refrigerado o en una condiciones especiales. 

';
COMMENT ON COLUMN PRODUCTOS.USER_NAME IS 'En este campo se registra el usuario que registró el producto por primera vez.';
...
GRANT EXECUTE
 ON PROCEDURE PERM_FRM_FACTURAR TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE PERM_FRM_FACTURAR TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE PRO_PERMISOS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE PRO_PERMISOS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_GET_CATEGORIA_ACTIVAS TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_GET_CATEGORIA_ACTIVAS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_GET_CATEGORIA_ACTIVAS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_GET_CLIENTES_SB TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_GET_CLIENTES_SB TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_GET_CLIENTES_SB TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_INSERT_DIRECCION TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_INSERT_DIRECCION TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_PRODUCTOS_FIND TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_PRODUCTOS_FIND TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_GENERALES TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_GENERALES TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_GET_CAJEROS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_GET_CAJEROS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_GET_CLIENTES_SB TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_GET_CLIENTES_SB TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_SELECT_PRODUCTOS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_PRODUCTOS_FIND TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_PRODUCTOS_FIND TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_PRODUCTOS_ID_CODIGO TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_PRODUCTOS_ID_CODIGO TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_TURNOS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_TURNOS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_TURNOS_POR_CAMPOS_PUBLICO TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_TURNOS_POR_CAMPOS_PUBLICO TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_TURNOS_POR_CAMPOS_PUBLICO TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_SELECT_USUARIOS_TAGS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_UPDATE_CATEGORIA TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_UPDATE_DIRECCION TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_UPDATE_DIRECCION TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_UPDATE_DIRECCION_CLIENTE TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_UPDATE_DIRECCION_CLIENTE TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_VALIDAR_USUARIO TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_VALIDAR_USUARIO TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_VS_PERMISOS_ASIGNADOS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_VS_PERMISOS_ASIGNADOS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_V_CATEGORIAS TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_V_CATEGORIAS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_V_CATEGORIAS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GET_CLIENTES TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GET_PERSONAS_ID TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GET_PERSONAS_ID TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA;

GRANT SELECT
 ON GET_PRIVILEGIOS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_E_S_SYS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_PERSONAS_ESTUDIANTES_VER TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_PERSONAS_ESTUDIANTES_VER TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_TRANSACCION_ACTUAL TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_TRANSACCION_ACTUAL TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_USUARIOS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_USUARIOS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA;
