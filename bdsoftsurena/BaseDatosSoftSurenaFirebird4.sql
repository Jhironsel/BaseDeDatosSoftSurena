...
SET TERM ^ ;
CREATE PROCEDURE SP_SELECT_V_M_FACTURAS
RETURNS (
    ID TYPE OF COLUMN V_M_FACTURAS.ID,
    ID_CLIENTE TYPE OF COLUMN V_M_FACTURAS.ID_CLIENTE,
    ID_CONTACTOS_TEL TYPE OF COLUMN V_M_FACTURAS.ID_CONTACTOS_TEL,
    ID_CONTACTOS_DIRECCIONES TYPE OF COLUMN V_M_FACTURAS.ID_CONTACTOS_DIRECCIONES,
    ID_CONTACTOS_EMAIL TYPE OF COLUMN V_M_FACTURAS.ID_CONTACTOS_EMAIL,
    ID_TURNO TYPE OF COLUMN V_M_FACTURAS.ID_TURNO,
    FECHA_HORA TYPE OF COLUMN V_M_FACTURAS.FECHA_HORA,
    TOTAL TYPE OF COLUMN V_M_FACTURAS.TOTAL,
    EFECTIVO TYPE OF COLUMN V_M_FACTURAS.EFECTIVO,
    ESTADO_FACTURA TYPE OF COLUMN V_M_FACTURAS.ESTADO_FACTURA,
    NOMBRE_TEMP TYPE OF COLUMN V_M_FACTURAS.NOMBRE_TEMP,
    USER_NAME TYPE OF COLUMN V_M_FACTURAS.USER_NAME )
SQL SECURITY DEFINER
AS 
BEGIN SUSPEND; 
END^
...
CREATE VIEW GET_CAJEROS (USER_NAME, ROL, PNOMBRE, SNOMBRE, APELLIDOS, 
     ESTADO, DESCRIPCION)
AS --
--
SELECT 
...
CREATE VIEW GET_CLIENTES (ID, ID_PROVINCIA, ID_MUNICIPIO, 
     ID_DISTRITO_MUNICIPAL, ID_CODIGO_POSTAL, ID_TIPO_SANGRE, 
     PERSONA, CEDULA, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, DIRECCION, 
...
INNER JOIN V_PERSONAS r ON r.ID = c.ID
INNER JOIN V_GENERALES g ON g.ID_PERSONA = c.ID
WHERE c.ID >= 0 AND r.ID >= 0 AND g.ID_PERSONA >= 0
/*

*/;
CREATE VIEW GET_DEUDAS (ID, ID_CLIENTE, ID_FACTURA, CONCEPTO, MONTO, 
     FECHA, HORA, ESTADO, P_NOMBRE, S_NOMBRE, APELLIDOS, CEDULA)
...
CREATE VIEW GET_DIRECCION_BY_ID (ID, ID_PERSONA, ID_PROVINCIA, 
     PROVINCIA, ID_MUNICIPIO, MUNICIPIO, ID_DISTRITO_MUNICIPAL, 
     DISTRITO_MUNICIPAL, ID_CODIGO_POSTAL, CODIGO_POSTAL, DIRECCION, 
     FECHA, ESTADO, POR_DEFECTO)
...
CREATE VIEW GET_D_FACTURAS (ID_FACTURA, ID_LINEA, ID_PRODUCTO, 
...
/**/;
CREATE VIEW GET_ESTUDIANTES_SV (ID, MATRICULA, CEDULA, PNOMBRE, 
...
/**/;
CREATE VIEW GET_PACIENTES (ID, ID_ARS, NONSS, ID_TIPO_SANGRE, CEDULA, 
     PNOMBRE, SNOMBRE, APELLIDOS, SEXO, FECHA_NACIMIENTO, 
...
*/;
CREATE VIEW GET_PRIVILEGIOS (USER_NAME, CEDENTE, PRIVILEGIO, 
     OPCION_PERMISO, NOMBRE_RELACION, NOMBRE_CAMPO, TIPO_USUARIO, 
     TIPO_OBJECTO, DESCRIPCION)
...
FROM V_TANDAS t;
CREATE VIEW GET_TEMPORALES (ID_FACTURA, NOMBRE_TEMP, PNOMBRE, 
     SNOMBRE, APELLIDOS, ID_TURNO, EFECTIVO, FECHA_HORA, 
...
*/;
CREATE VIEW GET_TURNOS (ID, ID_ALMACEN, TURNO_USUARIO, 
     FECHA_HORA_INICIO, FECHA_HORA_FINAL, ESTADO, MONTO_FACTURADO, 
     MONTO_DEVUELTO, MONTO_EFECTIVO, MONTO_CREDITO, NOMBRE_ALMACEN, 
     ID_FACTURA)
AS --
SELECT 
     r.ID, 
...
CREATE VIEW VS_PROCEDIMIENTOS (ID, PROCEDIMIENTO, PROPIETARIO, 
     SQL_SECURITY, DESCRIPCION)
...
FROM ANTECEDENTES 
WHERE ID >= 0;
CREATE VIEW V_ARS (ID, DESCRIPCION, COVERTURA_CONSULTA_PORCIENTO, 
     ESTADO, CANTIDAD_REGISTRO, USER_NAME, ROL)
AS SELECT 
     ID, 
     DESCRIPCION, 
     COVERCONSULTAPORC, 
...
FROM ASEGURADOS r
WHERE ID_PERSONA >= 0;
CREATE VIEW V_CARTONES_BINGO (ID, CARTON_HASH, FECHA_CREACCION, 
     MATRIZ_OBJ, ESTADO)
AS SELECT ID, CARTON_HASH, FECHA_CREACCION, MATRIZ_OBJ, ESTADO
FROM CARTONES_BINGO 
WHERE ID >= 0 AND CARTON_HASH <> 0;
CREATE VIEW V_CATEGORIAS (ID, DESCRIPCION, IMAGEN_TEXTO, 
     FECHA_CREACION, ESTADO, USER_NAME)
AS SELECT r.ID, r.DESCRIPCION, r.IMAGEN_TEXTO, r.FECHA_CREACION, r.ESTADO, r.USER_NAME
FROM CATEGORIAS r
WHERE r.ID >= 0;
CREATE VIEW V_CODIGOS_POSTALES (ID, IDPROVINCIA, LOCALIDAD, 
     CODIGO_POSTAL)
AS SELECT ID, IDPROVINCIA, LOCALIDAD, CODIGO_POSTAL
FROM CODIGOS_POSTALES 
WHERE ID >= 0;
CREATE VIEW V_CONSULTAS (ID, ID_PACIENTE, ID_CONTROL_CONSULTA, FECHA, 
     TURNO, ESTADO, USER_NAME)
AS SELECT ID, ID_PACIENTE, ID_CONTROL_CONSULTA, FECHA, TURNO, ESTADO, USER_NAME
FROM CONSULTAS
WHERE ID >= 0;
CREATE VIEW V_CONSULTAS_APROBADAS (ID, COD_AUTORIZACION, COSTO, 
     DESCUENTO, TOTALCOSTO, USER_NAME)
...
WHERE ID >= 0;
CREATE VIEW V_CONTACTOS_EMAIL (ID, ID_PERSONA, EMAIL, FECHA, ESTADO, 
     POR_DEFECTO)
AS SELECT ID, ID_PERSONA, EMAIL, FECHA, ESTADO, POR_DEFECTO
FROM CONTACTOS_EMAIL
WHERE ID >= 0;
CREATE VIEW V_CONTACTOS_TEL (ID, ID_PERSONA, TELEFONO, TIPO, FECHA, 
     ESTADO, POR_DEFECTO)
AS SELECT ID, ID_PERSONA, TELEFONO, TIPO, FECHA, ESTADO, POR_DEFECTO
FROM CONTACTOS_TEL 
WHERE ID >= 0;
CREATE VIEW V_CONTROL_CONSULTA (ID, USER_NAME, CANTIDAD_PACIENTE, 
     DIA, INICIAL, FINAL, ESTADO, USER_NAME_)
AS SELECT ID, USER_NAME, CANTIDADPACIENTE, DIA, INICIAL, FINAL, ESTADO, USER_NAME_
FROM CONTROL_CONSULTA
WHERE ID >= 0;
CREATE VIEW V_DEUDAS (ID, ID_CLIENTE, ID_FACTURA, CONCEPTO, MONTO, 
     FECHA, HORA, ESTADO)
...
WHERE ID >= 0;
CREATE VIEW V_D_ANALISIS (ID, ID_M_ANALISIS, LINEA, ID_T_ANALITICA, 
     OTRO)
AS SELECT ID, ID_M_ANALISIS, LINEA, ID_T_ANALITICA, OTRO
FROM D_ANALISIS 
WHERE ID >= 0;
...
FROM D_DEUDAS_PAGAS 
WHERE ID >= 0;
CREATE VIEW V_D_ENTRADA_PRODUCTO (ID_M_ENTRADA_PRODUCTO, LINEA, 
     ID_PRODUCTO, ENTRADA, COSTO, PRECIO, FECHA_VECIMIENTO, 
     OBSERVACION)
...
CREATE VIEW V_D_FACTURAS (ID_FACTURA, ID_LINEA, ID_PRODUCTO, PRECIO, 
     CANTIDAD)
...
CREATE VIEW V_D_RECETAS (ID_RECETA, LINEA, ID_MEDICAMENTO, CANTIDAD, 
     D_DOSIS)
AS SELECT ID_RECETA, LINEA, ID_MEDICAMENTO, CANTIDAD, D_DOSIS
FROM D_RECETAS 
WHERE ID_RECETA >= 0;
CREATE VIEW V_E_S_SYS (ID, UUID_BASEDATOS, NOMBRE_EMPRESA, FCHI, 
     FCHA, FCHV, DIAS_RESTANTES, TELEFONOS, DIRECCION, 
...
FROM E_S_SYS
WHERE ID >= 0;
CREATE VIEW V_GENERALES (ID_PERSONA, CEDULA, ID_TIPO_SANGRE, 
     ESTADO_CIVIL)
AS SELECT ID_PERSONA, CEDULA, ID_TIPO_SANGRE, ESTADO_CIVIL
FROM GENERALES 
WHERE ID_PERSONA >= -1;
CREATE VIEW V_GUIA_VIGILANCIA_DESARROLLO (ID, EDAD, 
...
FROM INSCRIPCIONES
WHERE ID >= 0;
CREATE VIEW V_MENSAJES (ID, ID_DOCTOR, ID_PACIENTE, HORA, FECHA, 
     MENSAJE, ESTADO)
AS SELECT ID, ID_DOCTOR, ID_PACIENTE, HORA, FECHA, MENSAJE, ESTADO
FROM MENSAJES 
WHERE ID >= 0;
CREATE VIEW V_METRICAS (ID, IDCONSULTA, FECHA, PESOKG, ESTATURAMETRO, 
     ESCEFALO, NF_DETECT, HALLAZGOS_POS, ID_DIAG, TX, COMPLEMENTO, 
...
FROM MUNICIPIOS 
WHERE ID >= 0;
CREATE VIEW V_M_ANALISIS (ID, ID_PACIENTE, FECHA_HORA_CREADA, 
     FECHA_HORA_VISTA, ROL, USER_NAME)
AS SELECT ID, ID_PACIENTE, FECHA_HORA_CREADA, FECHA_HORA_VISTA, ROL, USER_NAME
FROM M_ANALISIS 
WHERE ID >= 0;
CREATE VIEW V_M_ENTRADA_PRODUCTOS (ID, ID_PROVEDOR, ID_ALMACEN, 
     COD_FACTURA, FECHA_ENTRADA, ROL, USER_NAME)
AS SELECT 
     ID, 
     IDPROVEDOR, 
     ID_ALMACEN, 
...
CREATE VIEW V_M_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, 
     ID_CONTACTOS_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO, 
     FECHA_HORA, TOTAL, EFECTIVO, ESTADO_FACTURA, NOMBRE_TEMP, 
     USER_NAME)
...
FROM M_FACTURAS 
WHERE ID >= 0;
CREATE VIEW V_PERSONAS (ID, PERSONA, PNOMBRE, SNOMBRE, APELLIDOS, 
     SEXO, FECHA_NACIMIENTO, FECHA_INGRESO, FECHA_HORA_ULTIMO_UPDATE, 
...
WHERE ID >= 0;
CREATE VIEW V_PERSONAS_CLIENTES_ATR (ID, TOTAL_FACTURADO, 
     TOTAL_DEUDA, CANTIDAD_FACTURA, FECHA_ULTIMA_COMPRA, SALDO)
...
WHERE ID >= 0;
CREATE VIEW V_PERSONAS_PACIENTES_ART (ID, PESO_NACIMIENTO_KG, ALTURA, 
     PERIMETRO_CEFALICO, CESAREA, TIEMPO_GESTACION, USER_NAME, 
     MASA_CEFALICA)
...
CREATE VIEW V_PLAN_CUENTA_CONTABLE (ID, PAIS, CODIGO_CUENTA_CONTABLE, 
     NOMBRE_CUENTA, TIPO_CUENTA, NIVEL_CUENTA, USA_TERCERO, 
     CENTRO_COSTOS, PORCENTAJE_BASE, MONTO_BASE_MIN, DETALLE_CUENTA)
...
CREATE VIEW V_PRODUCTOS (ID, ID_CATEGORIA, CODIGO, DESCRIPCION, 
     EXISTENCIA, IMAGEN_TEXTO, NOTA, FECHA_CREACION, ESTADO, 
     USER_NAME, ROL)
...
AS SELECT ID, IDCONSULTA, FECHA, USER_NAME
FROM RECETAS 
WHERE ID >= 0;
CREATE VIEW V_RELACION_PADRE_ESTUDIANTE (ID_PADRE_O_MADRE, 
     ID_ESTUDIANTE)
...
CREATE VIEW V_SINTOMAS (ID, ID_PACIENTE, SINTOMAS, FECHA, HORA, NOTA, 
     USER_NAME)
AS SELECT ID, ID_PACIENTE, SINTOMAS, FECHA, HORA, NOTA, USER_NAME
FROM SINTOMAS
WHERE ID >= 0;
CREATE VIEW V_TANDAS (ID, ANNO_INICIAL, ANNO_FINAL, HORA_INICIO, 
     HORA_FINAL, LUNES, MARTES, MIERCOLES, JUEVES, VIERNES, SABADOS, 
     DOMINGOS, CANTIDAD_ESTUDIANTES, CON_EDAD, EDAD_MINIMA, 
     EDAD_MAXIMA, ESTADO)
...
CREATE VIEW V_TRANSACCION_ACTUAL (TNIDENTIFICADORTRANSACCION, 
     ...
     TNOLDESTACTIVETRANSACTION, TCMODOAISLAMIENTO, TCTIEMPOESPERA, 
     TCREADONLY, TCAUTOCOMMIT, TCAUTOUNDO, TNIDENTIFICADORESTADISTICAS
     )
AS SELECT
...
CREATE VIEW V_TURNOS (ID, ID_ALMACEN, TURNO_USUARIO, 
     FECHA_HORA_INICIO, FECHA_HORA_FINAL, ESTADO, MONTO_FACTURADO, 
...
FROM TURNOS 
WHERE ID >= 0;
CREATE VIEW V_T_ANALISIS (ID, NOMBRE_CORTO_ANALISIS, NOMBRE_ANALISIS, 
     NOTA)
AS SELECT ID, NOMBRE_CORTO_ANALISIS, NOMBRE_ANALISIS, NOTA
FROM T_ANALISIS 
WHERE ID >= 0;
CREATE VIEW V_USUARIOS (USERNAME, PNOMBRE, SNOMBRE, APELLIDOS, 
     ESTADO, ADMINISTRADOR, DESCRIPCION)
AS SELECT 
  CAST(U.SEC$USER_NAME AS D_USER_NAME)  AS USUARIO,
...
COMMENT ON PROCEDURE SP_SELECT_V_M_FACTURAS IS 'Procedimiento que muestra las facturas maestras del sistema de factura.

Muestra los campos de la vista/tabla M_FACTURAS.

Es utilizada para obtener los identificadores de las facturas.
';
SET TERM ^ ;
ALTER PROCEDURE SP_SELECT_V_M_FACTURAS
RETURNS (
    ID TYPE OF COLUMN V_M_FACTURAS.ID,
    ID_CLIENTE TYPE OF COLUMN V_M_FACTURAS.ID_CLIENTE,
    ID_CONTACTOS_TEL TYPE OF COLUMN V_M_FACTURAS.ID_CONTACTOS_TEL,
...
BEGIN
     FOR SELECT 
          ...
          a.ID_CLIENTE, 
          a.ID_CONTACTOS_TEL, 
          a.ID_CONTACTOS_DIRECCIONES, 
          a.ID_CONTACTOS_EMAIL, 
          a.ID_TURNO, 
          a.FECHA_HORA, 
          a.TOTAL, 
          a.EFECTIVO, 
...
';
COMMENT ON VIEW V_PERSONAS_ESTUDIANTES_VER IS 'Esta vista permite obtener el identificador, la matricula y el estado de un estudiante.
...
GRANT EXECUTE
 ON PROCEDURE SP_SELECT_V_M_FACTURAS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_SELECT_V_M_FACTURAS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
